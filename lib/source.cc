...
/*
 * Create a new instance of eventstream_source and return
 * a boost shared_ptr.  This is effectively the public constructor.
 */
eventstream_source_sptr 
eventstream_make_source (gr_vector_int out_sig, int nthreads, enum eventstream_queue_early_behaviors eb, enum eventstream_source_merge_behavior mb)
{
  return eventstream_source_sptr (new eventstream_source (out_sig, nthreads, eb, mb));
}
...
unsigned long long eventstream_source::time(){
    return d_time;
}
 ...
 */
eventstream_source::eventstream_source (gr_vector_int out_sig, int nthreads, enum eventstream_queue_early_behaviors eb, enum eventstream_source_merge_behavior mb)
  : gr::sync_block ("eventstream_source",
        gr::io_signature::make (MIN_IN, MAX_IN, 0),
        eventstream_make_io_signature (out_sig.size(), out_sig) ),
    d_merge_mode(mb),
    d_maxlen(ULLONG_MAX),
    d_time(0),
    n_threads(nthreads), // poke this through as a constructor arg
    qq(100), dq(100),
    eventstream_event_acceptor(eb)
{
    // create and dispatch handler threads
    for(int i=0; i<n_threads; i++){
        boost::shared_ptr<eventstream_source_thread> th( new eventstream_source_thread(pmt::PMT_NIL, event_queue, &qq, &lin_mut, &readylist, &qq_cond, out_sig) );
        threadpool.push_back( th );
    }

    // bind event_queue->append() callback function
    boost::function< bool(eventstream_eh_pair**) > f;
    f = std::bind1st(std::mem_fun(&eventstream_source::cb), this);
    ...
    ih = eventstream_make_handler_insert_vector();
...
// callback bound to the eventstream_queue add_item routine (executed for each new eh pair)
bool eventstream_source::cb(eventstream_eh_pair** eh){
    
    DEBUG(printf("eventstream_source::cb() executing.\n");)

    // pass eh pair to lockfree fifos (out to threads)
    eventstream_eh_pair * tp = *eh;
    qq.push(tp);
...
void eventstream_source::set_max(unsigned long long maxlen){
    d_maxlen = maxlen;
}
 ...
 */
eventstream_source::~eventstream_source ()
{
    //shutdown threads
    for(int i=0; i<n_threads; i++){
        threadpool[i]->stop();
...
int 
eventstream_source::work (int noutput_items,
			...
			gr_vector_void_star &output_items)
{
  ...
  for(int i = 0; i<readylist.size(); i++){
    ...
    if(e_time >= d_time + noutput_items){ // event starts after our current buffer area save for later
        ...
        if(e_time < d_time){ // if event starts in the past handle the behavior appropriately
            switch(event_queue->d_early_behavior){
                ...
                case ASAP:
                    ...
                    evt = event_args_add(evt, pmt::intern("eventstream::event_time") , pmt::from_uint64(d_time));
                    e_time = event_time(evt);
                    break;
                case RNDF:
                    // update event time to be a near future time
                    evt = event_args_add(evt, pmt::intern("eventstream::event_time") , pmt::from_uint64(d_time+rand()%noutput_items));
                    e_time = event_time(evt);
                    break;
                default:
        ...
        if( !event_has_field( (evt), eventstream::event_buffer ) ){
            perror("malformed event");
            }

        // buf_list is a pmt_list of pmt_blobs containing buffers for N output ports
        pmt_t buf_list = event_field( (evt), eventstream::event_buffer ); 
        ...
        if(pmt::length(message_subscribers(pmt::mp("events_scheduled")))){
            ...
            if(! pmt::eqv(pmt::dict_ref(dict, pmt::mp("eventstream::event_type"), PMT_NIL), pmt::mp("CONTINUATION"))){
                dict = pmt::dict_delete( dict, pmt::intern("eventstream::event_buffer") );
        ...
        if(e_length > item_copy){
            ...
            if(event_has_field(evt, ORIG_FLAG)){
                ...
                evt_c = event_args_add( evt_c, ORIG_FLAG, event_field( evt, eventstream::event_buffer ) );
            }
